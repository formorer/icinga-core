	...
	if(xpddefault_host_perfdata_file_processing_interval>0 && xpddefault_host_perfdata_file_processing_command!=NULL)
	  {
	    ...
	    event_args_ptr_t nullargs;
	    nullargs.anything=0;

	    schedule_new_event(EVENT_USER_FUNCTION,TRUE,current_time+xpddefault_host_perfdata_file_processing_interval,TRUE,xpddefault_host_perfdata_file_processing_interval,NULL,TRUE,callback,nullargs,0);
	  }
	...
	if(xpddefault_service_perfdata_file_processing_interval>0 && xpddefault_service_perfdata_file_processing_command!=NULL)
	  {
	    ...
	    nullargs.anything=0;
	     ...
	     schedule_new_event(EVENT_USER_FUNCTION,TRUE,current_time+xpddefault_service_perfdata_file_processing_interval,TRUE,xpddefault_service_perfdata_file_processing_interval,NULL,TRUE,callback,nullargs,0);
	  }
...
int xpddefault_preprocess_file_templates(char *template_name){
	char *tempbuf;
	int x=0;
	int y=0;
	...
	tempbuf=(char *)malloc(strlen(template_name)+1);
	if(tempbuf==NULL)
		return ERROR;
	strcpy(tempbuf,"");

	for(x=0,y=0;x<strlen(template_name);x++,y++){
		if(template_name[x]=='\\'){
			if(template_name[x+1]=='t'){
				tempbuf[y]='\t';
				x++;
			        }
			else if(template_name[x+1]=='r'){
				tempbuf[y]='\r';
				x++;
			        }
			else if(template_name[x+1]=='n'){
				tempbuf[y]='\n';
				x++;
			        }
			else
				tempbuf[y]=template_name[x];
		        }
		else
			tempbuf[y]=template_name[x];
	        }
