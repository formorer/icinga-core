...
int service_notification(service *svc, int type, char *not_author, char *not_data, int options){
	...
	if(notification_list!=NULL){
		...
		CHKASPRINTF(asprintf(&mac->x[MACRO_SERVICENOTIFICATIONNUMBER],"%d",svc->current_notification_number));
		...
		CHKASPRINTF(asprintf(&mac->x[MACRO_SERVICENOTIFICATIONID],"%lu",svc->current_notification_id));
	  ...
	  clock_gettime(CLOCK_REALTIME, &method_start_time); // nanoseconds
		...
		if(log_notifications==TRUE){
			...
			case NOTIFICATION_CUSTOM:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;CUSTOM ($SERVICESTATE$);%s;$SERVICEOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			case NOTIFICATION_ACKNOWLEDGEMENT:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;ACKNOWLEDGEMENT ($SERVICESTATE$);%s;$SERVICEOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			case NOTIFICATION_FLAPPINGSTART:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;FLAPPINGSTART ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			case NOTIFICATION_FLAPPINGSTOP:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;FLAPPINGSTOP ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			case NOTIFICATION_FLAPPINGDISABLED:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;FLAPPINGDISABLED ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			case NOTIFICATION_DOWNTIMESTART:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;DOWNTIMESTART ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			case NOTIFICATION_DOWNTIMEEND:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;DOWNTIMEEND ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			case NOTIFICATION_DOWNTIMECANCELLED:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;DOWNTIMECANCELLED ($SERVICESTATE$);%s;$SERVICEOUTPUT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
			default:
			  CHKASPRINTF(asprintf(&temp_buffer,"SERVICE NOTIFICATION: %s;%s;%s;$SERVICESTATE$;%s;$SERVICEOUTPUT$\n",cntct->name,svc->host_name,svc->description,command_name_ptr));
				break;
				}
...
int create_notification_list_from_service(icinga_macros *mac, service *svc, int options, int *escalated){
	...
	*escalated=escalate_notification;
	...
	CHKASPRINTF(asprintf(&mac->x[MACRO_NOTIFICATIONISESCALATED],"%d",escalate_notification));
...
int host_notification(host *hst, int type, char *not_author, char *not_data, int options){
	...
	if(notification_list!=NULL){
		...
		CHKASPRINTF(asprintf(&mac->x[MACRO_HOSTNOTIFICATIONNUMBER],"%d",hst->current_notification_number));
		...
		CHKASPRINTF(asprintf(&mac->x[MACRO_HOSTNOTIFICATIONID],"%lu",hst->current_notification_id));
...
int notify_contact_of_host(icinga_macros *mac, contact_ptr_t cntct, host *hst, int type, char *not_author, char *not_data, int options, int escalated){
	...
	clock_gettime(CLOCK_REALTIME, &method_start_time); // nanoseconds
		...
		if(log_notifications==TRUE){
			switch(type){
			case NOTIFICATION_CUSTOM:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;CUSTOM ($HOSTSTATE$);%s;$HOSTOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			case NOTIFICATION_ACKNOWLEDGEMENT:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;ACKNOWLEDGEMENT ($HOSTSTATE$);%s;$HOSTOUTPUT$;$NOTIFICATIONAUTHOR$;$NOTIFICATIONCOMMENT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			case NOTIFICATION_FLAPPINGSTART:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;FLAPPINGSTART ($HOSTSTATE$);%s;$HOSTOUTPUT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			case NOTIFICATION_FLAPPINGSTOP:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;FLAPPINGSTOP ($HOSTSTATE$);%s;$HOSTOUTPUT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			case NOTIFICATION_FLAPPINGDISABLED:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;FLAPPINGDISABLED ($HOSTSTATE$);%s;$HOSTOUTPUT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			case NOTIFICATION_DOWNTIMESTART:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;DOWNTIMESTART ($HOSTSTATE$);%s;$HOSTOUTPUT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			case NOTIFICATION_DOWNTIMEEND:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;DOWNTIMEEND ($HOSTSTATE$);%s;$HOSTOUTPUT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			case NOTIFICATION_DOWNTIMECANCELLED:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;DOWNTIMECANCELLED ($HOSTSTATE$);%s;$HOSTOUTPUT$\n",cntct->name,hst->name,command_name_ptr));
				break;
			default:
			  CHKASPRINTF(asprintf(&temp_buffer,"HOST NOTIFICATION: %s;%s;$HOSTSTATE$;%s;$HOSTOUTPUT$\n",cntct->name,hst->name,command_name_ptr));
				break;
				}
