 ...
 */
int grab_contact_macros_r(icinga_macros *mac, contact_ptr_t cntct){
...
int grab_contact_macros(contact_ptr_t cntct){
	...
	case MACRO_TOTALSERVICEPROBLEMSUNHANDLED:
		...
		if(mac->x[MACRO_TOTALHOSTSUP]==NULL){
			...
			for(x=MACRO_TOTALHOSTSUP;x<=MACRO_TOTALSERVICEPROBLEMSUNHANDLED;x++)
				my_free(mac->x[x]);
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSUP],"%d",hosts_up));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSDOWN],"%d",hosts_down));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSUNREACHABLE],"%d",hosts_unreachable));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSDOWNUNHANDLED],"%d",hosts_down_unhandled));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSUNREACHABLEUNHANDLED],"%d",hosts_unreachable_unhandled));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTPROBLEMS],"%d",host_problems));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTPROBLEMSUNHANDLED],"%d",host_problems_unhandled));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICESOK],"%d",services_ok));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICESWARNING],"%d",services_warning));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICESCRITICAL],"%d",services_critical));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICESUNKNOWN],"%d",services_unknown));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICESWARNINGUNHANDLED],"%d",services_warning_unhandled));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICESCRITICALUNHANDLED],"%d",services_critical_unhandled));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICESUNKNOWNUNHANDLED],"%d",services_unknown_unhandled));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICEPROBLEMS],"%d",service_problems));
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALSERVICEPROBLEMSUNHANDLED],"%d",service_problems_unhandled));
		}
...
int grab_datetime_macro_r(icinga_macros *mac, int macro_type, char *arg1, char *arg2, char **output){
	...
	case MACRO_TIMET:
	  CHKASPRINTF(asprintf(output,"%lu",(unsigned long)current_time));
		break;

#ifdef NSCORE
	case MACRO_ISVALIDTIME:
	  CHKASPRINTF(asprintf(output,"%d",(check_time_against_period(test_time,temp_timeperiod)==OK)?1:0));
	break;
	...
	case MACRO_NEXTVALIDTIME:
		...
		if(next_valid_time==test_time && check_time_against_period(test_time,temp_timeperiod)==ERROR)
			next_valid_time=(time_t)0L;
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)next_valid_time));
		break;
#endif
...
int grab_standard_host_macro_r(icinga_macros *mac, int macro_type, host *temp_host, char **output, int *free_macro){
	...
	*free_macro=TRUE;
	...
	case MACRO_HOSTSTATE:
		...
		break;
	case MACRO_HOSTSTATEID:
	  CHKASPRINTF(asprintf(output,"%d",temp_host->current_state));
		break;
	case MACRO_LASTHOSTSTATE:
		...
		break;
	case MACRO_LASTHOSTSTATEID:
			    CHKASPRINTF(asprintf(output,"%d",temp_host->last_state));
		break;
	case MACRO_HOSTCHECKTYPE:
			    CHKASPRINTF(asprintf(output,"%s",(temp_host->check_type==HOST_CHECK_PASSIVE)?"PASSIVE":"ACTIVE"));
		break;
	case MACRO_HOSTSTATETYPE:
			    CHKASPRINTF(asprintf(output,"%s",(temp_host->state_type==HARD_STATE)?"HARD":"SOFT"));
		break;
	case MACRO_HOSTOUTPUT:
...
#ifdef NSCORE
	case MACRO_HOSTATTEMPT:
			    CHKASPRINTF(asprintf(output,"%d",temp_host->current_attempt));
		break;
	case MACRO_MAXHOSTATTEMPTS:
			    CHKASPRINTF(asprintf(output,"%d",temp_host->max_attempts));
		break;
	case MACRO_HOSTDOWNTIME:
			    CHKASPRINTF(asprintf(output,"%d",temp_host->scheduled_downtime_depth));
		break;
	case MACRO_HOSTPERCENTCHANGE:
			    CHKASPRINTF(asprintf(output,"%.2f",temp_host->percent_state_change));
		break;
	case MACRO_HOSTDURATIONSEC:
	case MACRO_HOSTDURATION:
		time(&current_time);
		...
		if(macro_type==MACRO_HOSTDURATIONSEC)
		  {
		    CHKASPRINTF(asprintf(output,"%lu",duration))
		  }
		else{
			...
			duration-=(minutes*60);
			seconds=duration;
			CHKASPRINTF(asprintf(output,"%dd %dh %dm %ds",days,hours,minutes,seconds));
			}
		break;
	case MACRO_HOSTEXECUTIONTIME:
	  CHKASPRINTF(asprintf(output,"%.3f",temp_host->execution_time));
		break;
	case MACRO_HOSTLATENCY:
			    CHKASPRINTF(asprintf(output,"%.3f",temp_host->latency));
		break;
	case MACRO_LASTHOSTCHECK:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_host->last_check));
		break;
	case MACRO_LASTHOSTSTATECHANGE:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_host->last_state_change));
		break;
	case MACRO_LASTHOSTUP:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_host->last_time_up));
		break;
	case MACRO_LASTHOSTDOWN:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_host->last_time_down));
		break;
	case MACRO_LASTHOSTUNREACHABLE:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_host->last_time_unreachable));
		break;
	case MACRO_HOSTNOTIFICATIONNUMBER:
			    CHKASPRINTF(asprintf(output,"%d",temp_host->current_notification_number));
		break;
	case MACRO_HOSTNOTIFICATIONID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_host->current_notification_id));
		break;
	case MACRO_HOSTEVENTID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_host->current_event_id));
		break;
	case MACRO_LASTHOSTEVENTID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_host->last_event_id));
		break;
	case MACRO_HOSTPROBLEMID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_host->current_problem_id));
		break;
	case MACRO_LASTHOSTPROBLEMID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_host->last_problem_id));
		break;
#endif
	case MACRO_HOSTACTIONURL:
	...
	case MACRO_HOSTGROUPNAMES:
		...
		for(temp_objectlist=temp_host->hostgroups_ptr;temp_objectlist!=NULL;temp_objectlist=temp_objectlist->next){
			...
			CHKASPRINTF(asprintf(&buf1,"%s%s%s",(buf2)?buf2:"",(buf2)?",":"",temp_hostgroup->group_name));
			my_free(buf2);
			buf2=buf1;
			}
		if(buf2){
	...
	case MACRO_TOTALHOSTSERVICESCRITICAL:
		...
		if(mac->x[MACRO_TOTALHOSTSERVICES]==NULL){
			...
			CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSERVICES],"%d",total_host_services));
			my_free(mac->x[MACRO_TOTALHOSTSERVICESOK]);
				    CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSERVICESOK],"%d",total_host_services_ok));
			my_free(mac->x[MACRO_TOTALHOSTSERVICESWARNING]);
				    CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSERVICESWARNING],"%d",total_host_services_warning));
			my_free(mac->x[MACRO_TOTALHOSTSERVICESUNKNOWN]);
				    CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSERVICESUNKNOWN],"%d",total_host_services_unknown));
			my_free(mac->x[MACRO_TOTALHOSTSERVICESCRITICAL]);
				    CHKASPRINTF(asprintf(&mac->x[MACRO_TOTALHOSTSERVICESCRITICAL],"%d",total_host_services_critical));
			}
...
int grab_standard_service_macro_r(icinga_macros *mac, int macro_type, service *temp_service, char **output, int *free_macro){
	...
	*free_macro=TRUE;
	...
	case MACRO_SERVICESTATE:
		...
		break;
	case MACRO_SERVICESTATEID:
	  CHKASPRINTF(asprintf(output,"%d",temp_service->current_state));
		break;
	case MACRO_LASTSERVICESTATE:
		...
		break;
	case MACRO_LASTSERVICESTATEID:
			    CHKASPRINTF(asprintf(output,"%d",temp_service->last_state));
		break;
#endif
	case MACRO_SERVICEISVOLATILE:
			    CHKASPRINTF(asprintf(output,"%d",temp_service->is_volatile));
		break;
#ifdef NSCORE
	case MACRO_SERVICEATTEMPT:
			    CHKASPRINTF(asprintf(output,"%d",temp_service->current_attempt));
		break;
	case MACRO_MAXSERVICEATTEMPTS:
			    CHKASPRINTF(asprintf(output,"%d",temp_service->max_attempts));
		break;
	case MACRO_SERVICEEXECUTIONTIME:
			    CHKASPRINTF(asprintf(output,"%.3f",temp_service->execution_time));
		break;
	case MACRO_SERVICELATENCY:
			    CHKASPRINTF(asprintf(output,"%.3f",temp_service->latency));
		break;
	case MACRO_LASTSERVICECHECK:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_service->last_check));
		break;
	case MACRO_LASTSERVICESTATECHANGE:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_service->last_state_change));
		break;
	case MACRO_LASTSERVICEOK:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_service->last_time_ok));
		break;
	case MACRO_LASTSERVICEWARNING:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_service->last_time_warning));
		break;
	case MACRO_LASTSERVICEUNKNOWN:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_service->last_time_unknown));
		break;
	case MACRO_LASTSERVICECRITICAL:
			    CHKASPRINTF(asprintf(output,"%lu",(unsigned long)temp_service->last_time_critical));
		break;
	case MACRO_SERVICEDOWNTIME:
			    CHKASPRINTF(asprintf(output,"%d",temp_service->scheduled_downtime_depth));
		break;
	case MACRO_SERVICEPERCENTCHANGE:
			    CHKASPRINTF(asprintf(output,"%.2f",temp_service->percent_state_change));
		break;
	case MACRO_SERVICEDURATIONSEC:
	case MACRO_SERVICEDURATION:
		...
		break;
	case MACRO_SERVICENOTIFICATIONNUMBER:
	  CHKASPRINTF(asprintf(output,"%d",temp_service->current_notification_number));
		break;
	case MACRO_SERVICENOTIFICATIONID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_service->current_notification_id));
		break;
	case MACRO_SERVICEEVENTID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_service->current_event_id));
		break;
	case MACRO_LASTSERVICEEVENTID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_service->last_event_id));
		break;
	case MACRO_SERVICEPROBLEMID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_service->current_problem_id));
		break;
	case MACRO_LASTSERVICEPROBLEMID:
			    CHKASPRINTF(asprintf(output,"%lu",temp_service->last_problem_id));
		break;
#endif
	case MACRO_SERVICEACTIONURL:
	...
	case MACRO_SERVICEGROUPNAMES:
		...
		for(temp_objectlist=temp_service->servicegroups_ptr;temp_objectlist!=NULL;temp_objectlist=temp_objectlist->next){
			...
			CHKASPRINTF(asprintf(&buf1,"%s%s%s",(buf2)?buf2:"",(buf2)?",":"",temp_servicegroup->group_name));
			my_free(buf2);
			buf2=buf1;
			}
		if(buf2){
 ...
 */
int grab_standard_contact_macro_r(icinga_macros *mac, int macro_type, contact_ptr_t temp_contact, char **output){
#ifdef NSCORE
	contactgroup *temp_contactgroup=NULL;
	objectlist *temp_objectlist=NULL;
	char *buf1=NULL;
	...
	case MACRO_CONTACTGROUPNAMES:
		...
		for(temp_objectlist=temp_contact->contactgroups_ptr;temp_objectlist!=NULL;temp_objectlist=temp_objectlist->next){
			...
			CHKASPRINTF(asprintf(&buf1,"%s%s%s",(buf2)?buf2:"",(buf2)?",":"",temp_contactgroup->group_name));
			my_free(buf2);
			buf2=buf1;
			}
		if(buf2){
...
int grab_standard_contact_macro(int macro_type, contact_ptr_t temp_contact, char **output){
 ...
 */
int grab_contact_address_macro_r(icinga_macros *mac, int macro_num, contact_ptr_t temp_contact, char **output){
...
int grab_contact_address_macro(int macro_num, contact_ptr_t temp_contact, char **output){
...
int set_argv_macro_environment_vars_r(icinga_macros *mac, int set){
	...
	for(x=0;x<MAX_COMMAND_ARGUMENTS;x++){
	  CHKASPRINTF(asprintf(&macro_name,"ARG%d",x+1));
...
int set_custom_macro_environment_vars_r(icinga_macros *mac, int set){
	...
	if((temp_host=mac->host_ptr) && set==TRUE){
		for(temp_customvariablesmember=temp_host->custom_variables;temp_customvariablesmember!=NULL;temp_customvariablesmember=temp_customvariablesmember->next){
		  CHKASPRINTF(asprintf(&customvarname,"_HOST%s",temp_customvariablesmember->variable_name));
	...
	if((temp_service=mac->service_ptr) && set==TRUE){
		for(temp_customvariablesmember=temp_service->custom_variables;temp_customvariablesmember!=NULL;temp_customvariablesmember=temp_customvariablesmember->next){
		  CHKASPRINTF(asprintf(&customvarname,"_SERVICE%s",temp_customvariablesmember->variable_name));
	...
	if((temp_contact=mac->contact_ptr) && set==TRUE){
		for(temp_customvariablesmember=temp_contact->custom_variables;temp_customvariablesmember!=NULL;temp_customvariablesmember=temp_customvariablesmember->next){
		  CHKASPRINTF(asprintf(&customvarname,"_CONTACT%s",temp_customvariablesmember->variable_name));
...
int set_contact_address_environment_vars_r(icinga_macros *mac, int set){
	...
	for(x=0;x<MAX_CONTACT_ADDRESSES;x++){
	  CHKASPRINTF(asprintf(&varname,"CONTACTADDRESS%d",x));
...
int set_macro_environment_var(char *name, char *value, int set){
	...
	CHKASPRINTF(asprintf(&env_macro_name,"%s%s",MACRO_ENV_VAR_PREFIX,name));
